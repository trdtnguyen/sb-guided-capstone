...
class Cleaning:
    ...
    def data_correction_trade(self, filepath:str):
        ...
        arr_trade_dates = trade_final_df.select(trade_final_df['trade_dt']).distinct().rdd.flatMap(lambda row: row).collect()
...
c = Cleaning()
partition_dir = os.path.join(c.PROJECT_PATH, c.CONFIG['DATA']['PARTITION_PATH'])
partition_by=c.CONFIG['DATA']['PARTITION_LABEL']
